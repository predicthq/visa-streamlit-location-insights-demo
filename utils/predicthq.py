...
def fetch_features(lat, lon, radius, date_from, date_to, features=[], radius_unit="mi"):
    ...
    for feature in features:
        if "phq_attendance_"+feature.replace("-","_") in PHQ_ATTENDANCE_FEATURES:
            features_to_fetch.append("phq_attendance_"+feature.replace("-","_"))  
    ...
    if len(features_to_fetch) > 0:          
        ...
        features = phq.features.obtain_features(
            ...
            **{feature: True for feature in features_to_fetch},
        )
...
def fetch_event_counts(
    lat, lon, radius, date_from, date_to, categories=[], tz="UTC", radius_unit="mi"
):
    phq = get_predicthq_client()
...
@st.cache_data
def fetch_event_spend_total(lat, lon, radius, date_from, date_to, categories=[], tz="UTC", radius_unit="mi"):
    radius_value = f"{radius}{radius_unit}"

    spend_hospo=0
    spend_accom=0
    spend_trans=0
    if(any(item in categories for item in ATTENDED_CATEGORIES)):
        request_body_categories = {"active":{"gte":str(date_from),"lte":str(date_to)},
                "location":{"geo":{"lat":lat,"lon":lon,"radius":radius_value}}}    
        value = {"stats":["sum"],"phq_rank":{"gte":0,"lte":100}}
        for category in categories:
            if category in ATTENDED_CATEGORIES:
                request_body_categories["phq_spend_"+category.replace("-","_")+"_accommodation"] = value
                request_body_categories["phq_spend_"+category.replace("-","_")+"_transportation"] = value
                request_body_categories["phq_spend_"+category.replace("-","_")+"_hospitality"] = value
        
        r = requests.post(
            url="https://api.predicthq.com/v1/features/",
            json=request_body_categories,
            headers={
                "Authorization": f"Bearer {get_api_key()}",
                "Accept": "application/json",
            },
        ...
        for item in r.json()["results"]:
            if item.get("phq_spend_conferences_hospitality"):
            ...
            if item.get("phq_spend_expos_hospitality"):
            ...
            if item.get("phq_spend_sports_hospitality"):
            ...
            if item.get("phq_spend_community_hospitality"):
            ...
            if item.get("phq_spend_concerts_hospitality"):
            ...
            if item.get("phq_spend_festivals_hospitality"):
            ...
            if item.get("phq_spend_performing_arts_hospitality"):
